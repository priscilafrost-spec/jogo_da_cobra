...
const VELOCIDADE = parseInt(localStorage.getItem("snake_valocidade")) || 130;
...
function mudarDirecao(tecla) {
    if (tecla === "ArrouwUp" && direcao.y !== 1) {
        proximaDirecao = { x: 0, y: -1 };
    };

    if (tecla === "ArrouwDown" && direcao.y !== -1) {
        proximaDirecao = { x: 0, y: 1 };
    };

    if (tecla === "ArrouwLeft" && direcao.x !== 1) {
        proximaDirecao = { x: -1, y: 0 };
    };

    if (tecla === "ArrouwRight" && direcao.x !== -1) {
        proximaDirecao = { x: 1, y: 0 };
    };
}
